...
class Board():
    ...
    def isValidMove(self, space, value):
        ...
        elif value in (self.valsInBoxes[self.spaceToBox(space[0], space[1])]):
            return False
        
        elif (0 >= space[0] > self.n2 and 0 >= space[1] > self.n2):
            return False
    ...
    def evaluateSpace(self, space):
        ...
        constraints = set()
        constraints = constraints.union(self.valsInRows[space[0]])
        constraints = constraints.union(self.valsInCols[space[1]])
        constraints = constraints.union(self.valsInBoxes[self.spaceToBox(space[0], space[1])])
    ...
    def getMostConstrainedUnsolvedSpace(self):
        ...
        for i in self.unsolvedSpaces:
            unsolvedSpaces[i] = self.evaluateSpace(i)
...
class Solver:
    ...
    def solveBoard(self, board):
        ...
        constrainedSpaces = board.getMostConstrainedUnsolvedSpace()

        for j in range(1, board.n2 + 1):
            if board.isValidMove(constrainedSpaces, j):
                board.makeMove(constrainedSpaces, j)
                availableSolutions = self.solveBoard(board)
                if availableSolutions == True:
                    return availableSolutions
                else:
                    board.undoMove(constrainedSpaces, j)
        else:
            return False
...
if __name__ == "__main__":
    ...
    board = Board('/Users/aaronrusk/Desktop/B351/a2/example.csv')
    s = Solver()
    s.solveBoard(board)
    board.print()
